...
RSpec.describe Bump::FileParsers::Cocoa::CocoaPods do
  it_behaves_like "a dependency file parser"

  let(:files) { [podfile, lockfile] }
  let(:podfile) do
    Bump::DependencyFile.new(name: "Podfile", content: podfile_body)
  end
  let(:lockfile) do
    Bump::DependencyFile.new(name: "Podfile.lock", content: lockfile_body)
  end
